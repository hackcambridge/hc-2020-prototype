...
echo "MAILCHIMP_KEY=\"$MAILCHIMP_KEY\"" >> .env
echo "MAILCHIMP_DOMAIN=\"$MAILCHIMP_DOMAIN\"" >> .env
echo "MAILCHIMP_APPLICANT_LIST=\"$MAILCHIMP_APPLICANT_LIST\"" >> .env
echo "MAILCHIMP_PARTICIPANT_LIST=\"$MAILCHIMP_PARTICIPANT_LIST\"" >> .env
...
echo "* * * * * bitnami /opt/bitnami/php/bin/php /home/bitnami/htdocs/artisan schedule:run >> /dev/null 2>&1" > /etc/cron.d/mailing-list-update
